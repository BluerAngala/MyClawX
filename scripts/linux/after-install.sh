...
# Post-installation script for MyClawX on Linux
...
# Create symbolic link for MyClawX app binary
if [ -x /opt/MyClawX/clawx ]; then
    ln -sf /opt/MyClawX/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/MyClawX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/MyClawX/chrome-sandbox' || true
else
    chmod 0755 '/opt/MyClawX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/MyClawX/resources/apparmor-profile'
...
echo "MyClawX has been installed successfully."
