...
/**
 * Merge a MyClawX context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeMyClawXSection(existing: string, section: string): string {
...
/**
 * Detect and remove bootstrap .md files that contain only MyClawX markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairMyClawXOnlyBootstrapFiles(): Promise<void> {
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed MyClawX-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove MyClawX-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge MyClawX context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeMyClawXContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('MyClawX context directory not found, skipping context merge');
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      const merged = mergeMyClawXSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged MyClawX context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure MyClawX context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureMyClawXContext(): Promise<void> {
  let skipped = await mergeMyClawXContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeMyClawXContextOnce();
    if (skipped === 0) {
      logger.info(`MyClawX context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`MyClawX context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`MyClawX context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
